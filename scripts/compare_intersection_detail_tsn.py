...
from compare_core import (CompareSchema, normalize_value, run_compare,
                          keys_for, pair_occurrences_by_similarity, union_keys)
...
def compare(tsmis_path, tsn_path, out_path, events=None, confirm_overwrite=None,
    ...
    # Add the two-line 'Report View' replica (the flat Comparison sheet is untouched).
    # The TSN-only columns come from the raw TSN file (None for a normalized library);
    # locations come from the consolidated TSMIS — both aligned to the loaded rows.
    tsn_one = _tsn_onesided(tsn_path)
    tm_loc = _tsmis_locations(tsmis_path)
    schema = dataclasses.replace(
        _SCHEMA, extra_sheet_writer=lambda wb, ctx: _write_report_view(wb, ctx, tsn_one, tm_loc))

    return run_compare(schema, rows_t, rows_n, True, out_path,

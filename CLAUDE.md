...
2. **Login (once, or when the session expires):** Double-click
  `
  1. login (update login).bat` — opens a visible browser, the user
    ompletes SSO + MFA, then presses Enter to save the session into
...
4. **Consolidate (optional, repeatable):** Double-click
  `
  1. consolidate (combine reports).bat` — pick one report type and combine
    very per-route export into a single workbook under `output/consolidated/`.
...
1. run_export (main script).bat`:
2. Checks that `scripts\tsmis_auth.json` exists — if not, instructs the user
...
3. Shows a numbered menu:
  ...
  - `Q` → quit
4. Invalid choices loop back to the menu.
5. After a report is chosen, the Python entry point (`run_cli`) prompts for the
...
1. consolidate (combine reports).bat` follows the same pattern but runs no

auth check (the consolidator reads local files, not TSMIS).

Each `export_*.py` still has a `__main_`_ entry, so the BAT calls are
...
- `**scripts/consolidate_*.py`** — each exposes `consolidate(events, confirm_overwrite) -> ConsolidateResult` (console-free: logs via `Events`,
...
resetting the per-report tally and posting `("export_done", [(spec, RunResult), …])` at the end. Skip/Cancel are `threading.Event`s; login waits on a `done`
...
- **The engine was already route-aware:** `run_export(spec, events, *, routes=ROUTES)` and `run_export_parallel(..., routes=ROUTES)` take the route
...
standalone file exposing the same `consolidate(events=None, confirm_overwrite= None) -> ConsolidateResult` contract — console-free (log via `events.on_log`, ask
...
1. Add `if **name** == "**main**": from cli import run_consolidate_cli;

...
def test_settings_dedup_and_atomicity():
    ...
    with _patch(settings, "CONFIG_FILE", tmp / "config.json"):
        ...
        check("legacy path-only settings remain distinguishable for fail-closed migration",
              ...
              == r"C:\TSN\legacy.xlsx")
        check("counts-only defaults OFF when the setting is absent",
              settings.get_matrix_preview_only() is False)
        settings.set_matrix_preview_only(True)
        persisted_preview = json.loads(
            settings.CONFIG_FILE.read_text(encoding="utf-8"))
        settings.set_matrix_preview_only(False)
        check("counts-only persists on and clears back to the safe default",
              persisted_preview.get("matrix_preview_only") is True
              and settings.get_matrix_preview_only() is False)
...
def test_support_bundle_allowlist():
    ...
    # The bundle manifest uses the allowlist, not all_settings(). Every trigger (the
    # Settings button, validate-and-package, --collect-evidence) now writes its
    # manifest in ONE place, so the property is asserted where it lives.
    src = (ROOT / "scripts" / "evidence.py").read_text(encoding="utf-8")
    check("the bundle manifest uses support_bundle_settings(), never all_settings()",
          "settings.support_bundle_settings()" in src
          and "settings.all_settings()" not in src)
    gui = (ROOT / "scripts" / "gui_settings_api.py").read_text(encoding="utf-8")
    check("...and the Settings button no longer writes a manifest of its own",
          "manifest.txt" not in gui)

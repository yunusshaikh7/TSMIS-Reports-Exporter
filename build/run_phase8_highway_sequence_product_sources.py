#!/usr/bin/env python3
"""Build product Highway Sequence consolidated witnesses from private sources."""
...
import consolidate_highway_sequence as excel_consolidator  # noqa: E402
import consolidate_tsmis_highway_sequence_pdf as pdf_consolidator  # noqa: E402
...
OUTPUT_ROOT = VISUAL_ROOT / "phase8_highway_sequence_product_sources_r2"
EXCEL_SOURCE = PRIVATE_ROOT / "current_tsmis_excel"
PDF_SOURCE = PRIVATE_ROOT / "current_tsmis_pdf"

SOURCE_BINDINGS = {
    "excel": (252, 24_634_973, "31a13ebc388951fdcadbba69d9188218af4548dd56d68c91e09f96bcb41765c8", ".xlsx"),
    "pdf": (252, 39_236_260, "072e538e5ebcbf015ec719565f003fb72027973a11d63c42f123802d8856dfa7", ".pdf"),
}
...
def _manifest(root: Path, suffix: str) -> dict[str, object]:
    ...
    return {
        ...
        "manifest_sha256": _sha_bytes(wire), "members": entries,
    }
...
def _bind_source(label: str, root: Path) -> dict[str, object]:
    ...
    if (observed["files"], observed["bytes"], observed["manifest_sha256"]) != (
        ...
        raise WitnessError(f"{label} source binding drift")
    return observed
...
def main() -> int:
    ...
    excel_result = excel_consolidator.consolidate(
        day=None, input_dir=EXCEL_SOURCE, out_path=excel_output,
    ...
    pdf_result = pdf_consolidator.consolidate(
        day=None, input_dir=PDF_SOURCE, out_path=pdf_output,
    ...
    if converted_manifest["files"] != 252:
        raise WitnessError(f"PDF converted-member count {converted_manifest['files']} != 252")
    ...
    if any(
        sources[label][key] != source_after[label][key]
        for label in sources for key in ("files", "bytes", "manifest_sha256", "members")
    ):
    ...
    product_code = {
        ...
        for path in (
            SCRIPTS / "consolidate_highway_sequence.py",
            SCRIPTS / "consolidate_xlsx_base.py",
            SCRIPTS / "consolidate_tsmis_highway_sequence_pdf.py",
            SCRIPTS / "pdf_table_lib.py",
        )
    }
    result = {
        "audit": "Stage 8 Highway Sequence product consolidation witnesses",
        "sources": sources, "sources_after": source_after,
        "product_code": product_code,
        "excel": {
            "result": excel_serialized, "output": _identity(excel_output),
            "log_lines": len(excel_logs), "log_sha256": _sha_bytes(_json(excel_logs)),
            "logs": excel_logs,
        },
        "pdf": {
            "result": pdf_serialized, "output": _identity(pdf_output),
            "converted_members": converted_manifest,
            "log_lines": len(pdf_logs), "log_sha256": _sha_bytes(_json(pdf_logs)),
            "logs": pdf_logs,
        },
        "invariants": {
    ...
    print(
        "PASS Highway Sequence product source witnesses: "
        f"Excel {excel_output.stat().st_size:,} bytes; PDF {pdf_output.stat().st_size:,} bytes; "
...
if __name__ == "__main__":
    try:
        ...
        print(f"FAIL Highway Sequence product source witnesses: {exc}")
        raise SystemExit(1)

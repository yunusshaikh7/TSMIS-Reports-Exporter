#!/usr/bin/env python3
"""Run every production Intersection Detail triangle leg in isolation.
...
def main(argv: list[str] | None = None) -> int:
    ...
    excel_consolidated = (
        args.work_root / "intersection_detail_excel_consolidated.xlsx"
    )
    pdf_consolidated = (
        args.work_root / "intersection_detail_pdf_consolidated.xlsx"
    )
    converted = args.work_root / "converted_pdf"

    sys.path.insert(0, str(SCRIPTS_ROOT))
    import consolidate_intersection_detail  # type: ignore
    import consolidate_tsmis_intersection_detail_pdf  # type: ignore
    import compare_intersection_detail_tsn  # type: ignore
    import compare_intersection_detail_pdf  # type: ignore

    events = QuietEvents()
    excel_result = consolidate_intersection_detail.consolidate(
    ...
    pdf_result = consolidate_tsmis_intersection_detail_pdf.consolidate(
        events=events, confirm_overwrite=lambda _path: True,
    ...
    comparisons = {
        ...
        "pdf_vs_excel": _run_comparison(
            "pdf_vs_excel",
            compare_intersection_detail_pdf.TSMIS_PDF_VS_EXCEL,
    ...
    (args.work_root / "product-witness-result.json").write_bytes(encoded)

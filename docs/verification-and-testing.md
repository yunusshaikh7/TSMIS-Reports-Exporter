...
| `check_report_recipe.py` | **R3 (v0.19.0):** the "add a report family" recipe is PROVEN, not just documented. Confirms (1) Highway Detail/Summary sit at frozen stable ids 8/9, **export-enabled** (real `save_via_export_button` specs) yet still consolidate/compare-ABSENT; (2) a hypothetical Highway family registers through the catalog (display views, consolidator map, TSN entries); (3) a ~20-line stub comparator on `compare_tsn_common.run_files_compare` runs end-to-end; (4) `pdf_table_lib`'s writer emits the TSMIS-format route workbook. The extension points a new report family needs, kept known-good. |
| `check_validation.py` | **W1 (v0.19.0):** one-click validation (`scripts/validation.py`) runs the on-disk samples through the REAL matrix comparison path, records COUNTS/OUTCOMES/folder NAMES only (RM05), degrades instead of crashing on a bad family, honors `should_cancel`, and `evidence.collect` ships the manifest. **v0.19.1** adds the phantom-env guard: decoy `_tsn_input`/`comparisons` store folders must NOT be walked as environments (the field bug where the TSN drop folder became a phantom "environment"). |
...
| `check_compare_ramp_summary_tsn.py` | **vs-TSN Ramp Summary** (v0.17.0 reference AGGREGATE recipe): Category/Count schema + `key_field=0` + `has_route=False`, unique category keys (incl. TSN-only P/V + grand Total), the TSMIS loader SUMMING consolidated per-route columns (missing column → 0), and end-to-end that P compares 0-vs-N, the TSMIS-only Ramp Points metric lands in *Only in TSMIS*, and the familiar "Summary by Category" sheet renders — see [comparison-engine.md](comparison-engine.md) §9d |
| `check_compare_intersection_summary_tsn.py` | **vs-TSN Intersection Summary** (v0.17.0 AGGREGATE, one-sided divergence): 72-category union spec (unique keys/slugs; CONTROL J-P TSN-only, S/O/Q/R + INTERSECTION-TYPE R/C/P TSMIS-only via `Cat.sides`), the spec-driven block-walk mapper (incl. Rural/Urban `-O` parent disambiguation), the consolidated-workbook summing loader, and end-to-end that a TSN-only code lands in *Only in TSN* / a TSMIS-only code in *Only in TSMIS* (56 both / 10 / 6) — see [comparison-engine.md](comparison-engine.md) §9e |
| `check_compare_intersection_detail_tsn.py` | **vs-TSN Intersection Detail** (re-blessed to the July-2026 format, v0.22.0): the 33-field shared header ending `Xing Line Lgth`↔`X_CROSS_OVERRIDE`, the new consolidated positions + the **pre-update-workbook refusal** (header gate on the tail column → the re-export error, no output written), the S-crosswalk + boolean/zero-pad normalizations still MATCHING, Int St Eff-Date (TSN's bulk stamp) + the CS blank-vs-value gap COUNTED, Date of Record now matching, the data-driven Report-View soft/hard split (Date of Record + INT/Ctrl/Light = Major; Int St/ML/CS Eff-Date + Route Suffix = soft), the v3 sidecar sliced off on load, and `test_route_suffix_match` (base-route pairing + the suffix indicator) — see [comparison-engine.md](comparison-engine.md) §9f |
...
| `check_compare_highway_detail_tsn.py` | **vs-TSN Highway Detail** (v0.20.0 FLAT): the canonical roadbed-aware Post Mile key (TSMIS glued `R/L` ≡ TSN bare PM + `HG`; the equation marker excluded from the key and compared as the `PS` column — the two encodings PAIR, a marker disagreement flags a cell not a row), the NA `'A'`→blank / zero-pad / length-3-decimals / Med-WDA-glue / route-token normalizations, end-to-end that normalizations still MATCH while `RU Eff`-vs-`BEG_DATE` (structural) and a genuine lane change COUNT, the Notes + Report View sheets, and the normalized-library re-projection (stale-library repair) — see [comparison-engine.md](comparison-engine.md) §9f-2 |
...
| `check_visual_evidence.py` | **Visual evidence** (v0.21.0; Intersection Detail joined v0.22.0), the render-free logic layer: row registry + per-report source resolution + examples clamp + the per-report `availability()` shape, the `matrix.evidence_opts_for` caller gate, both adapters' LOCKSTEP pins vs their PDF consolidators, the HD TASAS two-line print regexes + span→box math, the ID fixed-window/max-overlap/flag-strip/LOC-tokenizer behavior + blank-cell template boxes, the verification projections, unique-key diff enumeration (the ID one mirroring compare_core's cell trim), both TSN loaders' sidecar contracts (`tsn_rows_with_dcr` row-identical to the locked raw loaders; a legacy library refuses with the rebuild hint), and the ensure_layout pdf/-folder chain for BOTH reports. The render stack itself is proven by the frozen self-test |
...
| `check_matrix.py` | the comparison-matrix engine (`scripts/matrix.py`): the 10-row enumeration (both HL + both Intersection + both Highway Detail formats, v0.20.0), mtime staleness, stable dateless paths, hidden row/env filters, the unified scoped rebuild list, real `compare_env` orchestration with a planted diff → counts read back + cached |
...
| `check_matrix_bridge.py` | the matrix `gui_api` bridge (stubbed workers): every method (baseline / report+env toggle / per-row + global mode / TSN file / scoped refresh / consolidate-TSN), single-task gate, and the "a cell export leaves a paused batch's manifest intact" invariant; **v0.18.1** the queue-phantom regression (a push-spy asserts the LAST pushed snapshot has the drained job cleared — `_try_start_next_matrix_job` pushes state on every queue mutation) |

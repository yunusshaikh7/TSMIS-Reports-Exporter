...
| `check_compare_env_intersection_summary.py` | **cross-env Intersection Summary** (v0.17.0 AGGREGATE-per-route env recipe): the adapter wiring (side_loader / sheet_name None / agg_header == IS_HEADER), its promotion to a full matrix row (in COMPARE_REPORTS + matrix_rows, removed from the TSN-only extra rows), and end-to-end that the aggregate path keys on Route, flags a genuine per-route diff, and emits the env-labelled workbook — see [comparison-engine.md](comparison-engine.md) §9c |

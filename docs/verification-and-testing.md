...
| `check_compare_intersection_detail_tsn.py` | **vs-TSN Intersection Detail** (v0.17.0 FLAT): PM key + route-from-LOCATION + PM/date normalization, the Y/N↔1/0 boolean normalization, the position-based TSMIS-consolidated loader, and end-to-end that a Y/1 boolean compares equal while a control-type change is a genuine diff, and the context columns (5 cross-street attrs + Date of Record) contribute ZERO diffs even when one side is blank + a Notes indicator sheet; **v0.18.1** `test_route_suffix_match` locks the "Roadbed"→"Route Suffix" column rename (`_split_route` base/suffix) — see [comparison-engine.md](comparison-engine.md) §9f |
...
| `check_visual_evidence.py` | **Visual evidence** (v0.21.0), the render-free logic layer: row registry + source resolution + examples clamp, the `matrix.evidence_opts_for` caller gate, the adapter's LOCKSTEP pins vs the PDF consolidator, the TASAS two-line print regexes + span→box math, the verification projections, unique-key diff enumeration, and the TSN loader's sidecar contract (`tsn_rows_with_dcr` row-identical to the locked raw loader; the v2 normalized sheet appends exactly District/County; a legacy library refuses with the rebuild hint). The render stack itself is proven by the frozen self-test |

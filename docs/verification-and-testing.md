...
| `check_intersection_gate.py` | the app-wide export-disable gate (`DISABLED_EXPORT_SUBDIRS`): as of **v0.18.1** it holds the **reserved Highway Detail/Summary** pair — `export_reports_status` SHOWS them greyed (per-report `disabled` flag) rather than hiding, the other N−2 reports stay enabled, `enabled_export_reports`/`report_library_info`/matrix exclude the pair, `start_*` reject a disabled report by its stable export-op key server-side (P3 — selection travels by key, not index), and the toggle behavior. (The gate formerly held Intersection, enabled since v0.16.1/v0.17.0.) |

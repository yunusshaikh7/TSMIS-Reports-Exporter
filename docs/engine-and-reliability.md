...
`run_export` (sequential): start Playwright → `new_authed_browser(p)` → `navigate_with_auth` → `require_signed_in` → `preflight(page, spec.label)` → loop over `routes` (default `ROUTES` from `common.py`). For each route:

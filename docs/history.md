...
| `v0.25.0` | Jul 9 | **Highway Sequence (PDF) fully integrated + the Intersection Summary July fix** — the first real work-PC print set (the same-day 7.9 bundle) un-blocked the staged path: census-first print parser (60,493/60,493 parse-back vs Excel; the print catches an Excel-dropped Description), PDF↔TSN + PDF↔Excel comparisons (the print shares TSN's equate convention, so it pairs BETTER than the Excel edition), both matrix rows, and the evidence adapter (raw-sourced TSN prints, like HL). Intersection Summary absorbed the site's `MASTARM`→`MASTERARM` rename via a parse-only alias + a loud section-partition tripwire |
| `v0.25.1` | Jul 9 | **Every edition, everywhere** — Ramp Summary gains its Excel sibling (the site Export button the app never wired) and Intersection Summary its print edition, so every enabled on-site report exports in both formats the site offers (both coalesce with their siblings); the dev site's brand-new Route History Table (an embedded SSRS report, no export flow) appears greyed as reserved groundwork at stable id 15 |
| `v0.25.2` | Jul 9 | **Hotfix** (field-driven, same evening) — a plain (non-fast, non-store) export of a coalesced Excel+PDF pair crashed instantly (`Path(None)` on the per-edition run dirs; latent since v0.19.2 — fast mode never coalesces and the store always overrides, so the user's first standard-mode pair run was the first field exercise); each None entry now falls back to its dated run folder, regression-locked |

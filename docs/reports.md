...
- **Full Highway Log (PDF) treatment (v0.17.3)** — consolidates AND compares, exactly like #4b. It's still **export-only on auto-consolidate** (absent from `_CONSOLIDATOR_BY_SUBDIR`, so B2 skips it; it needs a scratch converted_dir), but it has a Consolidate-tab consolidator (`consolidate_tsmis_intersection_detail_pdf`, "TSMIS Intersection Detail (PDF)") and three comparisons: cross-environment (`compare_env.INTERSECTION_DETAIL_PDF`, both sides PDF-parsed), TSMIS (PDF) vs TSN, and TSMIS (PDF) vs TSMIS (Excel) (`compare_intersection_detail_pdf`). It's a full row in both matrices. See [Report 6b consolidation/parsing](#report-6b--consolidation) below.
- Verified against a synthetic Print fixture + a headless `page.pdf()` (`build/check_fake_site.py` -> `test_intersection_detail_pdf_save`); the consolidator's parse is reconciled against the statewide PDF+Excel corpus (218/218 routes, 0 content differences) and the mapping is locked by `build/check_intersection_detail_pdf.py`. **Live-export verification against TSMIS is still pending** (the dev PC can't reach the site).

#### Report 6b — consolidation
The PDF is a bordered table where **each record is TWO physical rows** (`intd_renderRow`: a rowA of 21 cells + a rowB whose wide Description spans 4 grid columns). The site **zebra-shades alternate records**, and ONLY the shaded ones carry cell rectangles — so the consolidator derives the 21-column grid ONCE from the shaded rowA bands (the print layout is one table, so widths are uniform) and assigns EVERY text line to that grid, pairing each rowA (numeric Post Mile) with the rowB after it. rowA maps 1:1 to columns 0–20; rowB's Description (a merged window over grid cols 3–6) → col 21 and its fields → cols 22–35, with the **'Intrte S' / 'Intrte Route' pair SWAPPED** to match the Excel export's column order. The Description's only PDF-vs-Excel difference is whitespace (the PDF render collapses the Excel's source double-spaces), which compare_core's `_xl_trim` absorbs at compare time. Parser internals mirror [Highway Log PDF parsing](highway_log/pdf-and-tsn-parsing.md).
...
- `"TSMIS Intersection Detail (PDF)"` (`consolidate_tsmis_intersection_detail_pdf`, v0.17.3) -- reads the app's own "Intersection Detail (PDF)" export, day-aware (NOT a dropped folder), parsing the two-row cell-grid into the SAME 36-column format as the Excel export (the accurate substitute where the two sources disagree). Like the Highway Log (PDF) consolidator it needs a scratch converted_dir, so it is NOT in `_CONSOLIDATOR_BY_SUBDIR` (no auto-consolidate; the matrix drives it through a temp dir).
...
| Intersection Detail -- TSMIS (PDF) vs TSMIS (Excel) (v0.17.3, consistency check) | `compare_intersection_detail_pdf.TSMIS_PDF_VS_EXCEL` | files | env |

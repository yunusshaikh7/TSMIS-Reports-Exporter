...
- **⚠ TAXONOMY DIVERGENCE (confirmed on real data):**
  - **CONTROL TYPES — SIGNAL FOLD (2026-06-24, matches the Detail crosswalk):** A–I shared; the **TSN signal
    sub-types J/K/L/M/N/P** fold into the shared **`S - SIGNALIZED (incl. TSN J-P)`** category (via
    `summary_layout._CONTROL_SIGNAL_FOLD`, applied in `counts_from_rows` for BOTH sides), so Signalized
    compares directly (TSMIS S **2,713** vs TSN J–P summed **2,648**). Genuinely TSMIS-only (the TSN
    summary PDF has no such row): **R-YIELD ALL WAYS / O-PED HYBRID BEACON / Q-FLASH BEACON**.
  - **INTERSECTION TYPE:** F/S/Y/M/T/Z shared; **TSMIS-only R-ROUNDABOUT, C-OTHER CIRCULAR, P-MIDBLOCK PED, `+`-NO DATA**.
  - The **`+ no data` buckets the TSN PDF reports as 0 ARE compared** (e.g. NUM OF LANES `+` = 265 vs 0).
    LEFT CHAN `Y-CHANNELIZATION NOT SPECIFIED` stays TSMIS-only (no `Y` row in the TSN summary).
- **Canonical spec:** `summary_layout.INTERSECTION_SUMMARY_SPEC` = both taxonomies (**66** `(block, code)`
  categories after the signal fold) + grand Total, reusing the Ramp Summary AGGREGATE machinery +
  `extra_sheet_writer`; the familiar sheet carries a `notes` line documenting the fold.
- **Normalization:** key on block+code-letter (label text ignored); **CONTROL signal sub-types J–P → S**;
  rural/urban + lanes special-cased. **No ditto/roadbed.**
- **Drop folder / consolidator / comparator / golden check:** `tsn_library/intersection_summary/`
  ...
  signal fold + remaining one-sided codes via `Cat.sides`) · `check_compare_intersection_summary_tsn.py` +
  `check_consolidate_intersection.py`. Live in both matrices (`matrix.tsn_comparator_for("intersection_summary")`).
- **Approved counts (CANARY — 6.19 statewide set; never regress; signal fold 2026-06-24):** consolidator
  **218 routes → 16,473** (== the Intersection Detail row count ✓). Comparison: **66 categories — 58 both,
  8 only-TSMIS, 0 only-TSN** (signals folded → no TSN-only codes); **54 diff cells** (incl. Signalized
  2,713≠2,648 and num-lanes-`+` 265≠0), **4 identical**; TSMIS **16,473** vs TSN **16,626**.
...
- **Compare shape:** **FLAT**, **key = route + PM** (route from `LOCATION` `12 ORA 001` → `001`). Reconciled by hand on PM 0.204 (route 1). **CORRECTED findings (the planning-phase guess was wrong):**
  ...
  - ⚠ **Control-type crosswalk** (v0.17.5, 2026-06-24): TSN records signalized intersections under the legacy signal sub-types **J/K/L/M/N/P** (pretimed / semi- / full-actuated, 2- vs multi-phase), which TSNR/TSMIS collapses into the single category stored as `S`. Per the reference "TSNR - Intersection Control and Geometry Type", `_norm_control_type` folds both sides' signalized codes into one readable category **`Signalized`** so the sub-type split no longer flags — the word `Signalized` (vs the raw letter codes) shows the merge on the comparison sheet, and the Notes sheet documents it. Every other code (A/B/C/D/E/F/G/H/I/R/Z) is shared and compared unchanged. **This removed ~2,613 control-type diffs** (Control Type 2,614 → 1 statewide; total Excel-vs-TSN diffs 5,632 → 3,019). Geometry (INT Type) needs no crosswalk — both systems share F/M/S/T/Y/Z/R.
  - **`Date of Record`** is a TSMIS refresh date (`21-12-31`) ≠ TSN `DATE_REC` (`73-10-19`) → the **whole column differs**; under the compare-everything policy (2026-06-24) it is **COUNTED** like any field, with the Notes sheet commenting that the difference is structural (refresh-vs-record date), not per-row errors.
  - **Effective dates ARE compared, BY REPORT POSITION (v0.17.8, 2026-06-24 — supersedes the v0.17.7 value-alignment):** every date column is matched to the SAME column in the other report, not to TSN's nearest-value date. Mapping: **INT Type → `EFF_DATE_INT`, Control → `EFF_DATE_CT`, Lighting → `EFF_DATE_LT`** (geometry-vs-geometry, the systematic **~1-day** offset `73-10-18`↔`73-10-19`); **Mainline 1st → `EFF_DATE_ML`, Cross-street 1st → `CROSS_BEGIN_DATE`** (TSN's *original/geometry* dates — TSMIS shows its refresh date here, so these read as a **structural refresh-vs-original** difference like Date of Record, e.g. `21-12-31`↔`64-01-01`); **Mainline 2nd → `MAIN_EFF_DATE`, Int St → `EFF_DATE`** (the recent TSN dates; TSMIS holds a `2024` bulk stamp). **Nothing is greyed / shown-but-not-counted** any more — `context_fields = ()`; the two former `2024` context columns (pos 21 / pos 30) are now compared. The intersecting-route block + Main Line Length are also compared; the route **PM suffix reads from pos 35** (the `Xing Rte` value), not the always-blank pos 31.
  - TSMIS has an extra leading `S` column (pos 2, blank) that TSN lacks → the by-position offset differs from Ramp Detail.
  - ⚠ **Re-normalize the TSN library on READ** (`_load_tsn` → `_normalized_row`, fix 2026-06-24): the matrices compare against the cached `*_normalized.xlsx`, which `tsn_library.build_consolidated` **reuses, not rebuilds**, after a code change. So `_load_tsn` re-applies `_project` (idempotent) to the normalized rows — without it a stale pre-crosswalk library feeds raw `P` and flags **"Signalized ≠ P"**. Locked by `test_normalized_path_crosswalk`.
  - ⚠ **Roadbed-suffix route-label asymmetry** (found in the v0.17.0 audit, 2026-06-20): CA divided highways carry a roadbed letter (S/U) on the route name that **TSN keeps (`12 ORA 210U`) but TSMIS omits (`12 ORA. 210`)** for 7 routes (`005`/`008`/`010`/`014`/`058`/`101`/`178`/`210` family). **DECISION (user, 2026-06-20):** key on the **BASE route** so the same intersection still pairs across the label difference (≈31 intersections that previously dropped to one-sided), AND surface the suffix as a **compared `Roadbed` column** so a suffix-only difference is **flagged** (TSN `U` vs TSMIS blank) rather than dropped OR silently merged — *match-and-indicate*. `_split_route` returns `(base, suffix)`.
- **Normalization:** PM padding unified; route → **base** (roadbed suffix stripped for the key, surfaced as the `Roadbed` compared column); 2-digit-year dates → ISO (YY≥30 → 19YY); **`Y↔1 / N↔0` boolean normalization APPLIED** to mastarm / right-chan / lighting (shown as Y/N, documented in the Notes sheet); **control-type crosswalk APPLIED** (TSN signal sub-types J–P + TSMIS `S` → one `Signalized` category, v0.17.5); injection guard on Description. **No ditto.** **COMPARE-EVERYTHING + POSITION-ALIGNED policy (user, 2026-06-24; columns reordered to mirror the printed report in v0.17.7, position-aligned in v0.17.8):** every column present in both systems (incl. Date of Record, all 8 eff-dates, the 5 cross-street attrs, Main Line Length, the intersecting route, PR) is compared and counted (a mechanical diff) — **nothing is greyed or suppressed** (`context_fields = ()`); the Notes sheet *comments* on the columns that differ wholesale rather than hiding them. A second **`Report View`** sheet replicates the printed two-line record (every diff in red, white/grey record bands, per-record `Major`/`Diffs`) for visual inspection against the PDF.
- **Drop folder / consolidator / comparator / golden check:** `tsn_library/intersection_detail/` (raw XLSX → normalized via `tsn_load_intersection_detail.build_into`) · `consolidate_intersection_detail` (DONE) · `compare_intersection_detail_tsn` (FLAT; reads TSMIS **by position**, base-route key + `Roadbed` compared column, **`context_fields = ()` — nothing greyed/suppressed**, all 8 date columns compared **by report position** (ML/CS 1st → geometry `EFF_DATE_ML`/`CROSS_BEGIN_DATE`; ML 2nd / Int St → recent `MAIN_EFF_DATE`/`EFF_DATE`; INT/Control/Lighting geometry ~1-day), booleans normalized, **`_norm_control_type` signal crosswalk J–P/S → `S`**, `legend_writer` = Notes sheet, **`extra_sheet_writer` = the two-line "Report View" replica** that renders every diff in red with white/grey record bands) · `check_compare_intersection_detail_tsn.py`. Live in both matrices (reused by `compare_intersection_detail_pdf` for the PDF-vs-TSN side too).
- **Approved counts (CANARY — 6.19 statewide set; never regress; position-aligned + numeric-normalized + Report View, v0.17.8, 2026-06-24):** TSN normalized **16,626 rows / 216 routes** (== Intersection Summary total ✓). TSMIS consolidated **16,473** (218 routes). Comparison: **both 16,211**, only-TSMIS **262**, only-TSN **415**, **counted diff cells 163,353**, matched-with-diffs **16,211** (every matched row), identical **0**, routes both **211**, **routes one-sided NONE/NONE**. Per-field (counted): **eight whole-column date diffs at 16,211 each = 129,688** — Date of Record + the mainline & cross-street **1st AND 2nd** eff-dates (structural refresh-vs-original) + **INT Type / Control Type / Lighting** Eff-Date (the systematic ~1-day geometry offset); **CS Num Lanes 6,059 / CS Left Chan 6,049 / CS Right Chan 6,037 / CS Traffic Flow 6,020 / CS Mastarm 6,002** (cross-street; TSMIS-blank for ~37% of intersections → mostly completeness gaps); HG **958**, **Main Line Length 436** (was 1,398 before the zero-padding normalization), ML Num Lanes **411**, ML Left Chan **401**, ML Right Chan **358**, ML Traffic Flow **302**, ML Mastarm **290**, R/U **135**, City Code **127**, **Intrte Postmile 43** (was 98 before the trailing-zero normalization), **Roadbed 31**, Description **4**, INT Type **1**, Control Type **1** (post-crosswalk; was 2,614); **0**: PR, Lighting, Intrte Route, Intrte PM Prefix, Intrte PM Suffix. **Nothing greyed/context — every shared column is counted.** The mainline+identity subset (HG/ML/R-U/City/Roadbed/Desc/INT/Control) = **3,019** (unchanged — position alignment touched only the dates). PDF flavor = **163,361** (+8 Description trailing-tab cells — routes 025/033/111×4/299×2, the known tab-padding delta, unchanged by position alignment). Verified by the comparator's own counter + workbook readback + the full check suite.

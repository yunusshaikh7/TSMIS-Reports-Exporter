...
- **Normalization:** PM padding unified; route from `LOCATION`; 2-digit-year dates → ISO (YY≥30 → 19YY); **`Y↔1 / N↔0` boolean normalization APPLIED** to mastarm / right-chan / lighting (shown as Y/N, with a **Notes sheet + the indicator** the user asked for); Date of Record + the 5 cross-street attrs + PR are **context** (shown, not counted); injection guard on Description. **No ditto/roadbed.**
- **Drop folder / consolidator / comparator / golden check:** `tsn_library/intersection_detail/` (raw XLSX → normalized via `tsn_load_intersection_detail.build_into`) · `consolidate_intersection_detail` (DONE) · `compare_intersection_detail_tsn` (FLAT; reads TSMIS **by position**, `context_fields` = PR + Date of Record + 5 CS attrs, booleans normalized, `legend_writer` = Notes indicator) · `check_compare_intersection_detail_tsn.py`. Live in both matrices.
- **Approved counts (CANARY — 6.19 statewide set; never regress):** TSN normalized **16,626 rows / 216 routes** (== Intersection Summary total ✓). TSMIS consolidated **16,473** (218 routes). Comparison: **both 16,180**, only-TSMIS **293**, only-TSN **446**, matched-with-diffs **3,732**, **counted diff cells 5,520**, identical **12,448**, routes both **209**. Per-field (counted): Control Type **2603** (taxonomy divergence), HG **955**, ML Num Lanes **394**, ML Left Chan **388**, ML Right Chan **335**, ML Traffic Flow **295**, ML Mastarm **287**, R/U **133**, City Code **125**, Description **4**, INT Type **1**; **context columns 0** (cross-street blank-vs-N + Date of Record never counted; verified). Cross-street attrs are TSMIS-blank for ~37% of intersections (a real completeness gap, shown as context). Verified 3 ways (run summary + workbook readback + matrix normalized-workbook path).

...
- **Compare shape:** **FLAT**, **key = route + PM** (route from `LOCATION` `12 ORA 001` → `001`). Reconciled by hand on PM 0.204 (route 1). **CORRECTED findings (the planning-phase guess was wrong):**
  ...
  - **Effective dates ARE compared (v0.17.7, 2026-06-24):** the 5 attribute eff-dates — **INT Type → `EFF_DATE_INT`, Control → `EFF_DATE_CT`, Lighting → `EFF_DATE_LT`, Mainline → `MAIN_EFF_DATE`, Cross-street → `EFF_DATE`** (the RECENT TSN dates, not the historical `EFF_DATE_ML`/`CROSS_BEGIN_DATE`) — sit a **systematic 1-day apart** (TSMIS stores Dec 31, TSN Jan 1: `73-10-18`↔`73-10-19`, `21-12-31`↔`22-01-01`), so each flags on all 16,211 matched rows by design (raw comparison, user decision). **Two date columns are SHOWN-BUT-NOT-COUNTED (greyed, `context_fields`):** the 2nd mainline eff-date (pos 21) and the int-street eff-date (pos 30) are a uniform `2024` bulk stamp with no comparable TSN date. The intersecting-route block + Main Line Length are also compared; the route **PM suffix reads from pos 35** (the `Xing Rte` value), not the always-blank pos 31.
...
- **Normalization:** PM padding unified; route → **base** (roadbed suffix stripped for the key, surfaced as the `Roadbed` compared column); 2-digit-year dates → ISO (YY≥30 → 19YY); **`Y↔1 / N↔0` boolean normalization APPLIED** to mastarm / right-chan / lighting (shown as Y/N, documented in the Notes sheet); **control-type crosswalk APPLIED** (TSN signal sub-types J–P + TSMIS `S` → one `Signalized` category, v0.17.5); injection guard on Description. **No ditto.** **COMPARE-EVERYTHING policy (user, 2026-06-24; columns reordered to mirror the printed report in v0.17.7):** every column present in both systems (incl. Date of Record, the 5 eff-dates, the 5 cross-street attrs, Main Line Length, the intersecting route, PR) is compared and counted (a mechanical diff) **except 2 greyed `context_fields`** (`ML 2nd Eff-Date`, `Int St Eff-Date` — uniform `2024` bulk stamps with no TSN counterpart); the Notes sheet *comments* on the columns that differ wholesale rather than hiding them.
- **Drop folder / consolidator / comparator / golden check:** `tsn_library/intersection_detail/` (raw XLSX → normalized via `tsn_load_intersection_detail.build_into`) · `consolidate_intersection_detail` (DONE) · `compare_intersection_detail_tsn` (FLAT; reads TSMIS **by position**, base-route key + `Roadbed` compared column, **`context_fields = ("ML 2nd Eff-Date", "Int St Eff-Date")` + `context_fill="D9D9D9"`** greys them on matched rows, the 5 eff-dates compared raw vs the recent TSN dates, booleans normalized, **`_norm_control_type` signal crosswalk J–P/S → `Signalized`**, `legend_writer` = Notes sheet that documents every normalization) · `check_compare_intersection_detail_tsn.py`. Live in both matrices (reused by `compare_intersection_detail_pdf` for the PDF-vs-TSN side too).
- **Approved counts (CANARY — 6.19 statewide set; never regress; compare-everything + eff-dates v0.17.7, 2026-06-24):** TSN normalized **16,626 rows / 216 routes** (== Intersection Summary total ✓). TSMIS consolidated **16,473** (218 routes). Comparison: **both 16,211**, only-TSMIS **262**, only-TSN **415**, **counted diff cells 131,948**, matched-with-diffs **16,211** (every matched row), identical **0**, routes both **211**, **routes one-sided NONE/NONE**. Per-field (counted): **six whole-column date diffs at 16,211 each** — Date of Record (refresh-vs-record, structural) + the 5 eff-dates **INT Type / Control Type / Lighting / ML / CS Eff-Date** (the systematic 1-day offset) = **97,266**; **CS Num Lanes 6,059 / CS Left Chan 6,049 / CS Right Chan 6,037 / CS Traffic Flow 6,020 / CS Mastarm 6,002** (cross-street; TSMIS-blank for ~37% of intersections → mostly completeness gaps, **276** genuine value conflicts); **Main Line Length 1,398**; HG **958**, ML Num Lanes **411**, ML Left Chan **401**, ML Right Chan **358**, ML Traffic Flow **302**, ML Mastarm **290**, R/U **135**, City Code **127**, **Intrte Postmile 98**, **Roadbed 31**, Description **4**, INT Type **1**, Control Type **1** (post-crosswalk; was 2,614); **0**: PR, Lighting, Intrte Route, Intrte PM Prefix, Intrte PM Suffix. **Context (shown, NOT counted): ML 2nd Eff-Date 0, Int St Eff-Date 0.** The mainline+identity subset (HG/ML/R-U/City/Roadbed/Desc/INT/Control = **3,019**) breaks down as 799 genuine value conflicts + 2,166 TSMIS-missing + 54 TSMIS-extra. PDF flavor = **131,956** (+8 Description trailing-tab cells; Description 12 vs 4). Verified by the comparator's own counter + workbook readback + the full check suite.

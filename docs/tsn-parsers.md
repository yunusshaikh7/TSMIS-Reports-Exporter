...
> renders that arrangement in each Summary report's consolidated workbook AND a familiar-layout
> sheet on its comparison workbook (via the opt-in `CompareSchema.extra_sheet_writer`), so the
> output reads like the source for familiarity. **Detail / Sequence comparisons (Ramp Detail,
...
- **TSN format:** **PDF, statewide aggregate** (`Ramp Summary Statewide_TSN.pdf`, 3 pages; page 0 = policy boilerplate, page 1 = report params, page 2 = the data). **No per-route breakdown** — one statewide category-count table only. Same category blocks as TSMIS: Highway Groups, Ramp Types, On/Off Indicator, Population Groups. Footer: `Total number of Ramps: 15410`.
- **Compare shape:** **AGGREGATE.** Sum the 126 TSMIS per-route counts → statewide; compare **key = category code**, value = count (`has_route=False`). The TSN page-2 layout is two columns (left = Highway Groups / On-Off / Population; right = Ramp Types) — parse like `consolidate_ramp_summary.parse_pdf`'s word-position columns (`COLUMN_SPLIT_X`).
- **Normalization:** match category-code labels exactly between the TSN PDF text and the TSMIS code maps, or categories split into one-sided rows. **No ditto/roadbed.**
...
- **Compare shape:** **AGGREGATE.** Sum the 218 TSMIS per-route counts → statewide; compare **key = category code**, value = count. ⚠ category-label alignment between PDF and schema codes.

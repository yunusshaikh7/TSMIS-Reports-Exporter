...
- **Compare shape:** **FLAT**, **key = route + PM** (route from `LOCATION` `12 ORA 001` → `001`). Reconciled by hand on PM 0.204 (route 1). **CORRECTED findings (the planning-phase guess was wrong):**
  ...
  - **`Date of Record`** is a TSMIS refresh date (`21-12-31`) ≠ TSN `DATE_REC` (`73-10-19`) → the **whole column differs**; under the compare-everything policy (2026-06-24) it is **COUNTED** like any field, with the Notes sheet commenting that the difference is structural (refresh-vs-record date), not per-row errors. Eff-dates are not compared.
...
- **Normalization:** PM padding unified; route → **base** (roadbed suffix stripped for the key, surfaced as the `Roadbed` compared column); 2-digit-year dates → ISO (YY≥30 → 19YY); **`Y↔1 / N↔0` boolean normalization APPLIED** to mastarm / right-chan / lighting (shown as Y/N, documented in the Notes sheet); **control-type crosswalk APPLIED** (TSN signal sub-types J–P + TSMIS `S` → one `Signalized` category, v0.17.5); injection guard on Description. **No ditto.** **COMPARE-EVERYTHING policy (user, 2026-06-24):** there are **no context fields** — every column present in both systems (incl. Date of Record + the 5 cross-street attrs + PR) is compared and counted (a mechanical diff); the Notes sheet *comments* on the columns that differ wholesale rather than hiding them.
- **Drop folder / consolidator / comparator / golden check:** `tsn_library/intersection_detail/` (raw XLSX → normalized via `tsn_load_intersection_detail.build_into`) · `consolidate_intersection_detail` (DONE) · `compare_intersection_detail_tsn` (FLAT; reads TSMIS **by position**, base-route key + `Roadbed` compared column, **`context_fields = ()`** — nothing suppressed, booleans normalized, **`_norm_control_type` signal crosswalk J–P/S → `Signalized`**, `legend_writer` = Notes sheet that documents every normalization) · `check_compare_intersection_detail_tsn.py`. Live in both matrices (reused by `compare_intersection_detail_pdf` for the PDF-vs-TSN side too).
- **Approved counts (CANARY — 6.19 statewide set; never regress; compare-everything policy 2026-06-24):** TSN normalized **16,626 rows / 216 routes** (== Intersection Summary total ✓). TSMIS consolidated **16,473** (218 routes). Comparison: **both 16,211**, only-TSMIS **262**, only-TSN **415**, **counted diff cells 49,397**, matched-with-diffs **16,211** (every matched row — Date of Record differs on all), identical **0**, routes both **211**, **routes one-sided NONE/NONE**. Per-field (counted): **Date of Record 16,211** (whole column — refresh-vs-record date, structural), **CS Num Lanes 6,059 / CS Left Chan 6,049 / CS Right Chan 6,037 / CS Traffic Flow 6,020 / CS Mastarm 6,002** (cross-street; TSMIS-blank for ~37% of intersections → mostly completeness gaps, **276** genuine value conflicts), HG **958**, ML Num Lanes **411**, ML Left Chan **401**, ML Right Chan **358**, ML Traffic Flow **302**, ML Mastarm **290**, R/U **135**, City Code **127**, **Roadbed 31**, Description **4**, INT Type **1**, Control Type **1** (post-crosswalk; was 2,614), **PR 0**, **Lighting 0**. The mainline+identity subset (HG/ML/R-U/City/Roadbed/Desc/INT/Control = **3,019**) breaks down as 799 genuine value conflicts + 2,166 TSMIS-missing + 54 TSMIS-extra. PDF flavor = **49,405** (+8 Description trailing-tab cells). Verified by the comparator's own counter + workbook readback + the full check suite.

...
- **Compare shape:** **FLAT**, **key = route + PM** (route from `LOCATION` `12 ORA 001` → `001`). Reconciled by hand on PM 0.204 (route 1). **CORRECTED findings (the planning-phase guess was wrong):**
  ...
  - **`Date of Record`** is a TSMIS refresh date (`21-12-31`) ≠ TSN `DATE_REC` (`73-10-19`) → **exclude from counting** (context). Eff-dates show small real drift (1-day) — compared.
...
- **Normalization:** PM padding unified; route → **base** (roadbed suffix stripped for the key, surfaced as the `Roadbed` compared column); 2-digit-year dates → ISO (YY≥30 → 19YY); **`Y↔1 / N↔0` boolean normalization APPLIED** to mastarm / right-chan / lighting (shown as Y/N, with a **Notes sheet + the indicator** the user asked for); **control-type crosswalk APPLIED** (TSN signal sub-types J–P + TSMIS `S` → one `Signalized` category, v0.17.5); Date of Record + the 5 cross-street attrs + PR are **context** (shown, not counted); injection guard on Description. **No ditto.**
- **Drop folder / consolidator / comparator / golden check:** `tsn_library/intersection_detail/` (raw XLSX → normalized via `tsn_load_intersection_detail.build_into`) · `consolidate_intersection_detail` (DONE) · `compare_intersection_detail_tsn` (FLAT; reads TSMIS **by position**, base-route key + `Roadbed` compared column, `context_fields` = PR + Date of Record + 5 CS attrs, booleans normalized, **`_norm_control_type` signal crosswalk J–P/S → `Signalized`**, `legend_writer` = Notes indicator) · `check_compare_intersection_detail_tsn.py`. Live in both matrices (reused by `compare_intersection_detail_pdf` for the PDF-vs-TSN side too).
- **Approved counts (CANARY — 6.19 statewide set; never regress; post roadbed-match fix 2026-06-20):** TSN normalized **16,626 rows / 216 routes** (== Intersection Summary total ✓). TSMIS consolidated **16,473** (218 routes). Comparison: **both 16,211**, only-TSMIS **262**, only-TSN **415**, matched-with-diffs **3,763**, **counted diff cells 5,632**, identical **12,448**, routes both **211**, **routes one-sided NONE/NONE** (the roadbed fix removed the misleading "route 210 only in TSMIS / 210U only in TSN"). Per-field (counted): Control Type **2614** (taxonomy divergence), HG **958**, ML Num Lanes **411**, ML Left Chan **401**, ML Right Chan **358**, ML Traffic Flow **302**, ML Mastarm **290**, R/U **135**, City Code **127**, **Roadbed 31** (the divided-route suffix indicator), Description **4**, INT Type **1**; **context columns 0** (cross-street blank-vs-N + Date of Record never counted; verified). Cross-street attrs are TSMIS-blank for ~37% of intersections (a real completeness gap, shown as context). Verified 3 ways (run summary + workbook readback + matrix normalized-workbook path).

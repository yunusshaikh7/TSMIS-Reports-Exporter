...
- **Compare shape:** **FLAT**, **key = route + PM** (route from `LOCATION` `12 ORA 001` → `001`). Reconciled by hand on PM 0.204 (route 1). **CORRECTED findings (the planning-phase guess was wrong):**
  ...
  - ⚠ **Boolean encoding divergence:** the mastarm / L-R channelization / lighting attributes are **`Y/N` on TSN** but **`1/0` on TSMIS** (e.g. mastarm TSN `Y` ↔ TSMIS `1`; right-chan `N` ↔ `0`). Raw compare flags ~half the columns purely on encoding — needs a `Y↔1 / N↔0` normalization (pending user decision).
...
- **Normalization:** PM padding unified; route from `LOCATION`; 2-digit-year dates → ISO; `Y↔1 / N↔0` boolean normalization (pending decision); Date of Record excluded; injection guard on Description. **No ditto/roadbed.**
- **Drop folder / consolidator / comparator / golden check / approved counts:** `tsn_library/intersection_detail/` (raw XLSX → normalized) · `consolidate_intersection_detail` (DONE) · `compare_intersection_detail_tsn` (Phase 3d; FLAT recipe, read TSMIS by position, `context_fields` incl. Date of Record) · `check_compare_intersection_detail_tsn.py` · **canary TBD.**

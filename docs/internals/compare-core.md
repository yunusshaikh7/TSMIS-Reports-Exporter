...
> **The regression lock.** `compare_core` was extracted verbatim-then-parameterized from the approved TSMIS-vs-TSN Highway Log workbook. Its output is verified cell-for-cell (values, formulas, styles, conditional formatting, calc mode) against the pre-extraction engine. **Do not change formula or label TEXT** in this file without re-running that check. Almost every new behavior is gated behind a `CompareSchema` flag that defaults to OFF so the default path stays byte-identical — preserve that discipline. See `compare_core` module docstring (lines 1-34) and [../verification-and-testing.md](../verification-and-testing.md) for the golden `build/check_compare_*.py` guards.

---

## 1. The 30,000-foot control flow

`run_compare(sc, rows_t, rows_n, has_route, out_path, *, events, confirm_overwrite, mode, name_a, name_b, warnings)` (`compare_core.py:1650`) is the single entry point. Every caller loads + validates its two input files, turns each into a list of ROWS in the schema's column order, then calls this. The end-to-end pipeline:
...
├─ keys_t, keys_n = pair_occurrences_by_similarity(…)        # re-number DUPLICATE occ by similarity
...
└─ for m in modes:                                           # one workbook per flavor
     ...
     └─ wb.save(path)   (PermissionError → "open in Excel" message)
   → ConsolidateResult(status="ok", output_path, summary_lines, verdict)
```

The data is parsed/aligned/counted **once** (before the `for m in modes` loop, lines 1720-1757) and reused for both flavors — so "both" never disagrees and never re-aligns.
...
`sheet_names(has_route)` (`:174`) returns the fixed sheet list (`Summary, Spot Check, Comparison, [Routes,] Only in A, Only in B, A, B`) — used by `run_compare` for the length/uniqueness preflight.
...
2. For each group present on side A (`grp_t`):
   ...
   - Skip (keep file order) if `len(tis) * len(nis) > _PAIR_GROUP_CAP` (`100_000`, `:349`) — an absurd pile; counted in `capped` and logged once (`:454`).
   - Build a cost matrix `cost[ti][ni] = _row_diff_count(sc, rows_t[ti], rows_n[ni], off)` (`:433`).
   - `pairs = _min_cost_pairs(cost)` → list of `(a, b)` indices into `tis`/`nis`.
...
### `_min_cost_pairs` — exact-then-greedy assignment (`:369`)

Min-total-cost 1:1 assignment over the `nr × nc` matrix, returning `min(nr, nc)` `(row, col)` pairs:

1. **Flip** so rows are the smaller side (`flip = nr > nc`, transpose; un-flip the result at `:405`).
2. **Exact** when `math.perm(nc, nr) <= _PAIR_EXACT_PERMS` (`5040 == 7!`, `:350`): brute-force every injection `perm` of `nr` rows into `nc` columns via `itertools.permutations(range(nc), nr)`, with **branch pruning** — accumulate `total` and `break` the moment `total >= best_total` (valid because costs are non-negative, so a partial sum only grows, `:391`). Lexicographic iteration means the positional/file-order assignment is tried first, so **ties keep file order** — deterministic, and both flavors + the golden checks agree.
3. **Greedy fallback** above the threshold (`:396`): sort all `(cost, r, c)` ascending, take the lowest-cost pair whose row+col are both free, repeat until `nr` pairs. Bounds the rare pathological pile; not provably optimal but good enough there.

**Invariant the rest of the engine relies on:** the KEY/identity is unchanged — only the duplicate occurrence is re-numbered. The non-duplicate path is byte-identical (the approved Route-1 sample is untouched), and occurrence remains a **build-time literal** that every sheet `MATCH`es on (see §10), so the reassignment flows through both flavors with no formula change. Locked by `build/check_compare_dupmatch.py`.
...
- Per-column accessors: `data_col(field_idx)` (`:576`, `field_idx + 2 + off`), `field_col(field_idx)` (`:585`, via `_field_pos`), `field_pos` (`:581`), and `key_expr(r)` (`:589`) which builds the lookup key for Comparison row `r` (`$Route&"|"&$key&"|"&$occ`, matching the helper column format).
...
`count_diffs(sc, rows_t, rows_n, keys_t, keys_n, union, has_route)` (`:460`) computes everything the workbook's formulas will compute, in Python: overall totals (`both`, `t_only`, `n_only`, `diff_rows`, `identical`, `diff_cells`), per-field diff counts (`field_diffs`), per-route aggregates (`route`), and `first_diff_row` (the Spot Check default).

The per-cell comparison (`:498-506`) is the canonical semantic: for each `field_indices` column, `_xl_trim` both → skip on ditto (if enabled) → `_medwid_norm` for med-wid fields → `va != vb`. This **exact same logic** appears in three places that must agree: here, in `_row_diff_count` (the pairing cost), and in `_field_value` (the values-flavor literal). The `route` aggregates pre-count `t_rows`/`n_rows` from the full key lists (`:478-482`) then accumulate `locs`/`matched`/`withdiffs`/`cells` over the union.
...
- `normalize_value(v)` (`:275`) — applied by the LOADERS (not the engine) per loaded cell. Renders real `datetime`/`date`/`time` to fixed ISO strings, so the engine only ever sees text and Excel's locale-dependent date TRIM can't diverge from Python's `str()`. Everything else passes through.
- `_xl_trim(v)` (`:296`) — Excel TRIM: stringify (integer floats → int), collapse internal space runs, strip edges.
- `_medwid_norm(t)` (`:316`) — the Med Wid rule: `VALUE()` the whole code, else `VALUE()` all-but-last-char + keep that suffix, else raw — so `0Z`/`00Z` and `6V`/`06V` compare equal. Its formula twin is `_medwid_ref` (`:632`).
...
### Sheet 3 — Comparison (`_write_comparison`, `:828`)

The big sheet — one row per union key. CF set before rows (`:856-866`): red cells where the `≠` marker appears (`ISNUMBER(SEARCH(" ≠ ",…))`), yellow A-only rows, blue B-only rows, bold-red Diffs > 0.
...
- `Diffs` = `IF(status<>"Both","",SUMPRODUCT(--ISNUMBER(SEARCH(" ≠ ", field_range))))` — counts cells carrying the marker (matched cells show the value, so "non-blank" ≠ "different").
- field cells via `_field_formula`.

In the **values** branch (`:890`): the same shapes computed from `vals` — `_row_link_value` (build-time row number, no MATCH), literal `status`/`ndiff`, and `_field_value` per field.

#### `_DIFF_MARK = " ≠ "` (`:67`) — the load-bearing marker

A differing cell renders `a ≠ b`. The marker is detected by CF, the `Diffs` `SUMPRODUCT`, the per-field `COUNTIF`s, and the values-flavor `_DIFF_MARK in v`. **Changing this string breaks all of them at once.**

#### `_field_formula` (`:654`) and `_field_value` (`:678`)

`_field_formula` builds `=IF(status="<A> only", t, IF(status="<B> only", n, IF(eq, t, show_t&" ≠ "&show_n)))` — matched → the value, differ → `a ≠ b`, one-sided → that side's own value (so the row is still readable, not blank). `eq` is `t=n` (or the med-wid `_medwid_ref` form), wrapped by `_eq_with_ditto`. Excel's `IF` short-circuits, so the absent side's `INDEX` (row ref `""`) is never evaluated on one-sided rows. `_field_value` is its Python twin for the values flavor.
...
Each input copied verbatim with a leading `Comparison row` back-link (column A — `=HYPERLINK("#Comparison!u:u", u)` where `u = cmp_rows[i]`, a literal) and a trailing `Key (helper)` column. The helper key is a **build-time LITERAL** (`hk_t`/`hk_n`, see §10-literal below), guarded. Ditto tint/comment applied here when `ditto_resolver` is set.
...
Both flavors write LITERAL helper keys (`hk_t`/`hk_n` at `run_compare:1781`/`:1795`), not a live `COUNTIFS` occurrence. A live `COUNTIFS` **mis-numbers blank key fields** (Excel's blank-criterion quirk), so those rows' lookups failed and SELF-CHECK read CHECK. The literal always matches the Comparison sheet's stored occurrence #, and the row universe is build-time static by design (only field VALUES are live). The helper key is also injection-guarded (`_write_data_sheet:819`) — a key like `=X` would otherwise become a live formula and break the `=X|1` lookups AND split the two flavors.
...
- **`vals = {…}`** (values, `:1767`) — a precomputed model: `by_t`/`by_n` (key→row), `row_t`/`row_n` (key→data-sheet row number for the links), `routes_t`/`routes_n`, `counts` (the SAME `count_diffs` result), `n_t`/`n_n`, and the route-coverage sizes. The writers emit literal results.

**The guarantee:** both flavors derive every number from the ONE `count_diffs` call and the ONE alignment, via the same `_xl_trim`/`_medwid_norm`/`_is_plus_run` mirror that the live formulas implement (`_field_value` ↔ `_field_formula`, `_medwid_norm` ↔ `_medwid_ref`, `_is_plus_run` ↔ `_isditto_xl`). So the flavors **cannot disagree** — and the run summary `summary_lines` are built from the same `counts` too. Only the Spot Check sheet and SELF-CHECK rows stay live in the values workbook (they recount the literal sheets to prove internal consistency).

---

## 12. Write-path safety guards

Three guards, all preserving byte-identity on clean data:

1. **Formula-injection guard** — `is_formula_injection(value)` (`:714`) is True for text starting with `= + - @` (`_FORMULA_LEAD`, `:711`). `_styled(…, guard=True)` (`:726`) forces such a value to a STRING cell (`c.data_type = "s"`) so Excel shows it verbatim and never executes it. Applied to user-text cells (data-sheet values, helper keys, the key/route id cells, Routes' route-id cell) but **never to HYPERLINK cells** (they must stay formulas — the `guard_set` excludes `link_cols`, `:914-918`/`:1254-1258`). The value is kept byte-for-byte; only the cell TYPE changes, so equal sides still compare equal.
2. **`normalize_value`** (§9) — canonicalizes dates at LOAD so the two flavors can't diverge on locale-dependent date formatting.
...
`ConsolidateResult.verdict` (`events.py:127`) is `"match"` ONLY when `not (incomplete or not matches)` — so a clean match is NEVER certified when files were skipped. The GUI greens its result dialog only on `"match"` and titles the `⚠` case "Comparison incomplete". Match ⟺ zero differing cells AND zero one-sided rows AND zero skipped inputs.

---

## 15. Extension points

### Add a field to an existing CompareSchema without breaking the regression lock
...
4. Re-run `build/check_compare_*.py` — the no-warnings, non-Highway-Log output must stay byte-identical.

Do NOT change any existing formula or label TEXT in `compare_core` — the golden checks compare the literal output.
...
`group` is one of `COMPARE_GROUPS` ids — independent of `kind`, so input plumbing is untouched. **Don't hand-roll workbook output** — build a `CompareSchema` and call `run_compare`; that's the approved, regression-locked style for free. Accept `mode` even if you only implement one flavor.
...
- **`_DIFF_MARK = " ≠ "`** is referenced by CF, `SUMPRODUCT`, `COUNTIF`, and the values mirror — changing it silently breaks diff detection everywhere.
- **Occurrence is a build-time literal**, not a live `COUNTIFS` — blank key fields would otherwise mis-number it (Excel quirk) and break SELF-CHECK.
...
- **The Python mirror and the live formula must stay in lockstep** — three copies of the per-cell diff logic (`count_diffs`, `_row_diff_count`, `_field_value`) plus the formula side (`_field_formula`, `_medwid_ref`, `_isditto_xl`). Touch one, audit all.

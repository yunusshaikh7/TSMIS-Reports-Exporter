...
| DA5 | **The Clean Road build still uses the OLD block-eff-date rule — probably wrong there too** | DA1 proved `min()` wrong for the report by 20+ points, and Highway Detail PRINTS `THY_LEFT_ROAD_EFF_DATE`, so the same rule is very likely right for the TSN table the clean-road build reproduces. NOT changed: those columns are counted in the clean-road vs-TSN comparison and its blessed `CRH-SW-E2/E3` canary, and this dev box has no staged TSN extract to measure against. Owner-only — build both ways, score the three columns against the TSN extract, adopt `primary_eff=True` there if it wins. Canary re-bless either way. |
| DA6 | **Row boundaries are still not aligned** | The report build produces 51,064 records against the export's 51,327, and the merge pass still removes 3,585 rows rather than the ~0 an aligned segmentation would give. Something still splits where the report does not, and — since DA1 made adjacent eff dates agree more often — something now merges where it does. Deserves the census-first treatment DA1 got: the boundary sources are enumerable, so measure which ones the export actually draws. |

...
| E7 | **The startup Edge sign-in check never says whether it WORKED** (owner, 2026-08-20) | `_maybe_active_env_check` logs "Checking the saved sign-in in the background…" on start and `_on_active_env_done` closes with a bare "Background sign-in check finished." — outcome-free on purpose ("Outcome detail stays in the log file (quiet by design)"). So the pane says a check ran and never whether the saved session is good, which is the one thing the user wants at startup. The payload already carries everything needed: `via_device`, `signed_in`, `had_file` and `reason` (the same fields the Edge chip reads). Make the finish line state the outcome — signed in via the saved session / via device SSO / not signed in + why. Keep it ONE line and keep the failure wording UI-neutral per the core-is-console-free rule. `scripts/gui_api.py`. |

...
| [comparison-engine.md](comparison-engine.md) | You're in `compare_core` — the regression lock + harness, the two flavors, key-field / roadbed key / duplicate-pairing, ditto non-asserting, the verdict / incompleteness contract, write-path safety, the three comparison families, the **visual-evidence decoration** (§13 — `visual_evidence` + `evidence_highway_detail`). |

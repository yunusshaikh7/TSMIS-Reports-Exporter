...
> - **CMP-AUD-109 (`49e42f8`, CI green) — PARTIAL:** `_publish_evidence_set` commits the workbook first;
>   if it can't reach canonical (the common case — the workbook open in Excel), `_divert_images` diverts
>   the new images too, so BOTH old stay at canonical, BOTH new go to .new — never a new-wb/old-images
>   mix. + cancellation rechecked at the commit boundary + honest `status` (promoted/diverted) & actual
>   committed paths. Reuses the tested `_write_workbook`/`_swap_dir` primitives (low churn). **Residual:**
...
>
> **Gate 136/136 + 8 JS · ~219 closed (~91%).** BIG-1 progress — CI-verified:

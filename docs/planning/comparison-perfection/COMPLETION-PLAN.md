...
| **Findings** | **241 total · ~219 CLOSED (Resolved/Remediated) · ~22 open** (~91% closed by count; the correctness-critical core is done, the open tail is Phase-5/7 hardening + the evidence arc + non-product instrument work). **Buckets:** A ✅ · **B ✅** · **C ✅** · **D ✅** · **E ✅ (matrix/GUI lifecycle) — M6+M7+M8 COMPLETE, M9 3/6** (035/040/075 ✅; 085/098/115 Phase-5/Stage-10/owner-deferred) · **F STARTED (BIG-1 evidence arc) — 107/110/112 ✅, 106 partial** (+ the owner layout feature); 109 (workbook/images two-phase atomicity) + 108/208/209 (published-cell rebuild) remain. **G/H/I** remaining (HD normalization M12, IS/HSL M13, instruments). Vendor-blocked (never infer): 045/192/HL-county. |
...
its findings back-to-back, then update this roadmap.
...
> ### ▶ RESUME HERE (2026-07-18 — BIG-1 evidence arc: layout feature + 107 + 110 + 112 + 106(partial))
>
> **Gate 136/136 + 8 JS · code tip `6b889f1` · ~219 closed (~91%).** BIG-1 progress — CI-verified:
> - **CMP-AUD-110 (`3b82fc1`, CI green):** `_capture_evidence_identity` freezes an on-demand evidence
>   job's targeting (baseline/day-source/dest/TSN-selections/examples/layout) at ENQUEUE;
>   `_dispatch_evidence_job` uses that frozen identity, never live settings. Test in
>   `check_matrix_ownership.evidence_identity_checks` (freeze under A, change to B, assert A).
> - **CMP-AUD-112 (`145b039`, CI green):** `generate()` digests the candidate PDFs' bytes BEFORE parse
>   (`_pdf_content_digests`) and `_ensure_pdf_content_unchanged` re-verifies before commit — catches a
>   same-size/same-mtime swap the `(dev,inode,size,mtime)` tripwire can't. Only the sampled subset.
> - **CMP-AUD-106 (`6b889f1`, CI pending→verify) — PARTIAL:** the no-differing-columns path retires the
>   now-stale prior evidence (`_retire_stale_evidence` — guarded, quarantine-then-delete). **Still open:**
>   no-example/toggle-off/restart paths + the durable per-comparison evidence manifest (with 109).
>
> **NEXT in BIG-1 — the two HARDEST remain:** **109** = evidence workbook + images as ONE transaction
> (a two-phase-commit / rollback refactor of `_write_workbook`+`_swap_dir`+`generate()`'s publish — the
> most-hardened code; quarantine the old workbook too so a failed image swap rolls BOTH back; recheck
> cancel/identity at the commit boundary; return structured promoted/diverted/preserved/failed). Then
> **108/208/209** = drive evidence from the PUBLISHED Comparison cells + pairing trace, not a
> re-execution (the CLAUDE.md-flagged HSL end-to-end gap; exhaustive locator; all source roles/flavors;
> Stage-10-coupled) — **210 own mini-plan.** Do 109 as its own focused session (commit-machinery risk).
>
> ### ▶ (2026-07-18 — earlier: layout feature + CMP-AUD-107 CLOSED)
>
> **Two clean, CI-verified units earlier this day:**
> - **Owner feature — evidence workbook layout (`f0a1d0e`, CI green):** a Settings-backed choice
>   (`get/set_evidence_layout` ∈ side-by-side / stacked / both, default side-by-side) threaded through
>   the toggle-driven + on-demand + day paths; `visual_evidence` renders ONLY the chosen layout(s) (tabs
>   AND loose PNGs) and splits the example sheets into **one tab per comparison column** (`_column_image_sheets`
>   + `_sheet_name` legal/unique Excel names; `_write_workbook(layout=…)`). New `set_evidence_layout` bridge
>   endpoint + a `<select>` on both matrix pages (mock parity, #mock-verified).
> - **CMP-AUD-107 CLOSED (`217d02f`, CI pending→verify):** every evidence adapter now enumerates via the
>   engine's own `compare_core.compared_cell` verdict. HD was the verified defect (raw `cht._s`, no Excel
>   TRIM / Med-Wid fold / context suppression); its `enumerate_diffs` uses `compared_cell` and `project()`
>   applies `_xl_trim` so an example round-trips. ID (already `_xl_trim`/empty-context) folded on
>   byte-identically; RD's parallel skip lists replaced by the live per-flavor schema (`_schema_for`). Real
>   corpus (252-route HD): NEW ⊆ OLD provably; NEW == OLD == 198,752 (latent here); shipped adapter
>   reproduces 198,752. Red→green (HG-whitespace + Med-Wid) pinned in `check_visual_evidence`.
>
> **NEXT in BIG-1 (bucket F evidence arc):** the transaction/identity findings — **112** (parse↔render byte
> snapshot), **110** (queued evidence bound to immutable comparison identity), **106** (stale evidence
> retired on clean/fail/cancel via a generation manifest) + **109** (workbook+images one transaction). Then
> the deep **108/208/209** published-cell/pairing-trace rebuild (drive evidence from the committed
> Comparison cells + exhaustive locator; 210 own mini-plan). Start with 112 — most self-contained.
>
> ### ▶ (2026-07-18 — 8th + 9th marathons: M8 COMPLETE (8/8) · M9 PARTIAL (3 closed, 3 deferred))
>
...
> finding's "reject as partial" path). Census (HSL 252 + RD 126 PDFs): every real headerless page is
> a genuine cover/legend/trailer (0-1 anchor words, 0 postmile tokens) → no-op on real data, clean
> render stays COMPLETE. Shared `pdf_table_lib.page_has_postmile`; red→green + clean multi-page no-op
> in `check_pm_code_vocabulary`. **BUCKET B is now all but complete — only 061 (ID grid-scan
> cancellation, P2, mechanical, touches 5 evidence adapters) remains. NEXT: 061, then bucket E
> (082/084/088/089 matrix/cache lifecycle) / G (source-semantics; HD-Excel + HL raw county
> VENDOR-BLOCKED) / H (Stage-8 instruments).**
>
> **DONE 2026-07-18 (5th marathon, bucket B HD): CMP-AUD-051/052/053 CLOSED (`ab8d103`, CI
> SHA-verified), offline gate 134/134. 053 is a REAL CORRECTNESS FIX** — the
> census surfaced a 054-missed corruption: the reprinted "Acc-Cont Eff" header wraps as "ACC-" then
> a bare "CONT" line, which the parser consumed as a record's line-2 (garbage "C"/"ONT" in the
> roadbed columns, no Description) while the real line-2 orphaned. Recognizing "CONT" (+ dashed
> group headers) as furniture pairs each line-1 with its real line-2 — **7 records across 5 routes
> (018/101/110/152/395) corrected** to the printed Description + roadbed data (cell-for-cell), 2 hard
> orphans → PARTIAL. Old-vs-new (git-stash, 252 PDFs): count unchanged 51,201, ONLY those 7 rows
> moved (sha256 83fd8fb7→f8cf36d0). 052 (THEAD anchored on LEFT/RIGHTROADBED, not bare ROADBED/
> MEDIAN) byte-identical; 051 already closed by 054 `_is_line1`. **Downstream: the HD PDF-vs-Excel /
> PDF-vs-TSN legs lose ≤7 false-diff records — re-measure on the next full HD comparison pass
> (Excel-vs-TSN 48,644 UNAFFECTED).** Red→green in `check_highway_detail_pdf`. **NEXT in bucket B:
> 055 (HSL/RD damaged repeated header — `census_hslrd_055.py` ready) + 061 (ID grid-scan
> cancellation — mechanical, touches 5 evidence adapters).**
>
> **DONE 2026-07-18 (5th marathon, bucket B ID): CMP-AUD-056/057/058/059/060/062 CLOSED in one
> commit (`c6f72a6`, CI SHA-verified success), offline gate 134/134.** The
> Intersection Detail PDF parser now reconciles every data shape or escalates to PARTIAL —
> `_is_rowB` requires the integer AND a Description (058: numeric furniture can't hijack a record);
> a leading rowB is counted (057); a wrapped Description continuation is counted (056); vestigial
> 21st-column data escalates + is kept in diagnostics (060); a legacy postmile alongside current
> rows escalates (059); and a page whose own band grid diverges from the document median past
> `_PAGE_GEOM_TOL` is flagged (062). **Census-first (217 statewide 7.9 PDFs): every scenario is
> 0-occurrence; byte-identical rows old-vs-new (16,459/16,459, sha256 `5104861d…`), all counters 0
> — a discrepancy-safe no-op** (the ID grid is genuinely uniform: 0.000pt max per-page delta, so
> unlike HD-054 no per-page recovery is needed). Anomalies escalate completion only, never the
> file-count fields (CMP-AUD-064). Red→green + clean-render no-op + e2e escalation in
> `check_intersection_detail_pdf`. **NEXT in bucket B: 051/052/053 (HD parser — census running) +
> 055 (HSL/RD damaged header) + 061 (ID grid-scan cancellation).**
>
> **DONE 2026-07-18 (4th marathon): 073+074 (`d140cfc`) → 083 (`05cd0d8`) → 086 (`373203c`) → 072
> (`c244fc4`), all CI-green, offline gate 134/134.**
> - **072** (bucket E): `renderCompareDirs` fired `get_compare_folders` with no sequencing → a slow
>   response for an abandoned recipe stomped the current one (A→B→A). Added a `compareDirsSeq`
>   generation token + recipe-key snapshot (mirrors `refreshConsDest`'s `consDestSeq`) + a
>   `compareDirsLoading` flag holding Start disabled while a discovery is unresolved. NEW
>   `check_compare_dirs_race.js` (vm-sandbox, deferred out-of-order promises; red→green). #mock
>   confirmed the normal path still populates + enables.
> - **073+074** (UI tier): the classic Compare-tab file pickers were extension-blind (always
>   `*.xlsx`) and told every recipe "per-route OR consolidated". Added a census-confirmed
>   input-profile registry to `report_catalog` (std / hl / summary_tsn) — `pick_compare_file(side,
>   key)` now offers the raw TSN PDF on the two Summary-vs-TSN recipes' TSN side and Excel
>   elsewhere; the hint renders each recipe's real per-side shape (shipped via
>   `file_a_shape`/`file_b_shape` + mock.js). Guidance only; #mock-verified all three profiles.
> - **083** (bucket E): Matrix presence/mtime/discovery counted ANY file (a `~$lock`/`notes.txt`
>   folder read as an export). New `artifact_store.is_report_data_file` wired into all four scanners
>   (report_library ×2, day/baseline `_folder_newest_mtime`); census 6,897/6,897 real files kept.
>   `fingerprint` deliberately KEEPS conservative `_is_excluded` inclusion (near-match `.zlib` must
>   still bust freshness) — the allowlist answers "is this an export?", not "did anything change?".
> - **086** (bucket I): regenerated the reports.md COMPARE_REPORTS table from the catalog (20→29
>   rows, self groups fixed), fixed the 10→12 counts, corrected the "disabled gate empty" prose in 4
>   docs to name Route History, and made the work-PC live-verify set the 15 enabled reports (not 16).
>   New `check_docs_capability` pins it all to the catalog.
>
> **NEXT — heavy-census bucket B** (051/052/053/055 HD/HSL/RD parser robustness + 056–062 ID PDF) —
> each needs a statewide pdfplumber census in the background + Monitor, real DISCREPANCY RISK (a
> too-strict gate could false-reject a legit page), so census EXHAUSTIVELY. Or bucket **E** (more
> matrix/GUI lifecycle: 072 stale-folder-discovery race, 082 stale formula twins, 084 semantic cache
> invalidation, 088/089 …) / **G** (source-semantics — HD-Excel + HL raw county VENDOR-BLOCKED,
> never infer) / **H** (Stage-8 instruments). Pick by tractability; census-first on any new gate.
>
> ---
...
> EXHAUSTIVELY. **G** (source-semantics — **HD-Excel + HL raw county VENDOR-BLOCKED, never infer**).
> **E** (matrix/day/GUI lifecycle) largest cluster; **F** evidence arc (210 DEFERRED); **H** Stage-8
> instruments (non-product). Pick by tractability; census-first on any new gate.
>
> **DONE 2026-07-18: CMP-AUD-054 CLOSED (`d1d5ccc`) — the live HD-PDF fallback-grid corruption.**
> Band-less-line-1 pages parsed on a document-median grid that fits no page, shifting every field
> (route 005 p7: PM 005.009 → blank Length / Date=000.083, certified COMPLETE). Census-first: the
> statewide grid sweep (252 PDFs / 4,452 pages) proved the 10-cell line-1 grid is the EXACT merge of
> the 25-cell line-2 base edges at `_L1_FROM_L2_EDGES` on all 3,664 both-band pages (0 exceptions),
> so a band-less-line-1 page recovers its line-1 geometry from its OWN line-2 band — 17 pages /
> 13 routes parse faithfully (005.009 + 16 more digit-for-digit; old-vs-new route 005 3,068→3,070,
> the 2 removed rows were the median's corrupt versions, the 4 recovered match the raw print). The
> document median is gone for data; 15 routes' rect-less final records escalate to PARTIAL (see the
> owner flag above). Hermetic `test_fallback_recovery` (monkeypatched pdfplumber). Gate 132/132 +
> ruff, CI-green. Full re-bless: comparison-canary-bindings.md.
>
...
> blank headers then sliced rows to that width, silently dropping data beneath a trailing blank
> header. Now each refuses a row with a nonblank cell beyond the declared width — compare_env skips
> loudly (incompleteness channel), HL raises. Census: 1,316 real flat exports carry 0 data beyond
> the trimmed header (branch never fires; confirmed e2e on 252 HD exports, 0 refusals). Red→green
> in both checks. `e482827`, gate **132/132** + ruff clean.
>
> **DONE 2026-07-18: CMP-AUD-039 + 043 CLOSED (Summary-by-Category typed flags + snapshot).** The
> shared `summary_layout._render` (Ramp + Intersection `Summary by Category` sheet) re-derived its
> flag from an independent numeric delta and emitted literal values with no snapshot disclosure —
> the last open 039 slice + the aggregate half of 043. 039: each category now flags from
> `compared_cell` (the SAME verdict count_diffs already computed on those rows → agreement by
> construction; no-op on real integer counts per CMP-AUD-021). 043: unmistakable "build-time
> snapshot; do NOT recalculate" label (the finding's accepted values-only-snapshot path). Red→green
> on a `5` vs `"05"` fixture (delta says equal, typed verdict says different, sheet follows the
> verdict) + snapshot-label gates in both family checks. `732f9c5`, gate **132/132** + ruff clean.
>
> **DONE 2026-07-18: CMP-AUD-009 CLOSED (numeric identity key parity).** `keys_for` keyed on
> raw `str()`, so a float `5.0` alignment key never met its `"5"` twin — a false one-sided split
> BEFORE values compared (the value path already treats them equal via `_xl_trim`). New
> `_key_text` mirrors `_xl_trim`'s numeric/bool canon at the bare-`str()` else branch only;
> whitespace/case stay significant (identity ≠ display normalization). Step-0 census: the
> bare-`str()` path is reachable only by cross-env HD/HD-PDF/ID-PDF/HSL-PDF, whose 128,226 real
> key cells are 0 numeric / 0 whitespace → shipped `_key_text` == old `str()` byte-for-byte
> (no-op on real data; only latent numeric keys change, a correct fix). `db38c22`, gate
> **132/132** + ruff clean.
>
> **DONE 2026-07-18 (post-compact marathon start): CMP-AUD-038 CLOSED (date normalizer).**
> The shared TSN date normalizer `iso_date` prefix-matched with no calendar validation, so
> trailing junk (`02/25/1976 junk`) was truncated to a clean date and impossible dates
> (`02/31/1976`, non-leap `02/29/1900`) were faked into plausible ISO strings. Now it
> `re.fullmatch`-es each documented form + calendar-validates via a shared `_valid_ymd`
> (`datetime.date` probe); junk/impossible input is preserved VERBATIM as a visible diff.
> Discrepancy-safe two ways: the strict fullmatch set ⊆ the old prefix-match set, and the
> shipped normalizer == old on ALL 121,464 distinct real date cells (RD+ID, both editions;
> 0 valid dates moved). Red→green in `check_compare_tsn_common` (leap/impossible/trailing/
> timestamp/2-digit). `2507257`, gate **132/132** + ruff clean.
>
> **DONE 2026-07-18: CMP-AUD-064 CLOSED (PDF anomaly counts).** The HSL-PDF + RD-PDF
> consolidators put their unparsed-line count (`loud`) into the file-level `skipped_inputs`,
> so one PDF with three malformed lines reported three skips. Treated `loud` like the
> already-correct `bad_tokens` path — escalate completion only, record line counts in a
> structured `producer_extra['parse_anomalies']` — and added a shared
> `pdf_table_lib._clamp_input_counts` capping skipped/failed to discovered inputs. Real
> 3-route RD-PDF stays COMPLETE/skipped=0/no-anomalies; red→green (1 PDF/3 bad lines →
> skipped 3→0). `3c28bcd`, gate **132/132** + ruff clean.
>
> **DONE 2026-07-18: CMP-AUD-015 CLOSED (label fix).** The four base Intersection
> comparison labels (summary/detail × env/tsn) carried a wrong `TSAR:` (Ramp-family)
> prefix; Intersection's export/consolidate entries + PDF-variant labels never did.
> Dropped it on the four, synced the frozen-catalog baseline + `mock.js` + architecture/
> reports doc tables, and added a one-directional guard (no non-Ramp entry may start
> `TSAR:`) so it can't regress. `af70443`, gate **132/132** + ruff clean. Marathon-batched
> quick win between the heavier finds.
>
> **DONE 2026-07-18: CMP-AUD-068 + 069 CLOSED (Report View parity + PDF diagnostics).**
> **068** — the Highway Detail PDF-vs-TSN comparison omitted the two-line 'Report View'
> replica the Excel-vs-TSN leg builds (the ID half was already closed by CMP-AUD-239).
> Extracted `compare_highway_detail_tsn.add_report_view` as the shared per-call composer;
> the Excel `compare()` routes through it and the PDF `TSMIS_PDF_VS_TSN` opts in
> (`report_view=True` + `_schema_for`); the same-source PDF-vs-Excel keeps NONE (TSN-specific
> soft/ADT semantics don't apply to two TSMIS renders). Real-corpus: a PDF-vs-TSN compare
> builds a populated Report View (120,384 rows, route 001), parity with Excel; schema-parity +
> red→green in `check_highway_detail_pdf`. `011d8c1`. **069** — the Ramp Detail PDF wrapper
> didn't forward its `side_a`/`side_b` to `run_files_compare`, so a missing PDF-vs-Excel second
> file said "The TSN file doesn't exist" (default roles). Forward the flavor's own labels (like
> ID/HD); red→green existence-message test. `782b203`. Offline gate **132/132** + ruff clean.
> **NEXT: continue buckets B/E/G/H/I** — B (HD/ID PDF-parser robustness 051–062+044), the
> label/diagnostic tail (015/064/014), source-semantics (G), matrix/GUI (E), instrument (H).
>
> **DONE 2026-07-18: CMP-AUD-071 CLOSED — the Ramp Summary comparator now
> validates its route universe.** The vs-TSN comparator summed the consolidated workbook's
> category columns without requiring any route rows or checking route identity/uniqueness, so
> a header-only workbook certified a phantom zero universe and two Route-001 rows carrying 5
> each summed silently to 10 (the comparator-side twin of the consolidation-side CMP-AUD-050).
> Fix: Intersection Summary's `_validate_route_universe` (CMP-AUD-183) is promoted to the shared
> `compare_tsn_common.validate_route_universe` and BOTH aggregate comparators wire to it —
> require ≥1 valid normalized route, reject blank/duplicate, and reconcile against the producer's
> ordered `route_census`; the Ramp Summary consolidator now records that census in its outcome
> sidecar (mirroring IS), so a dropped/added/reordered/renamed row after consolidation is
> detectable. Census: the real 126-route ssor-prod consolidation records 001..980 and the
> comparator emits the verified note (total_ramps=15216). Red→green route-universe suite
> (header-only/blank/dup-identical/dup-conflicting/dropped/reordered) in
> `check_compare_ramp_summary_tsn`. `e850937`, offline gate **132/132** + ruff clean, CI-green.
> **NEXT: continue buckets B/E/G/H/I** (068 Report View parity in flight; then HD/ID parser
> robustness, matrix/GUI lifecycle, source-semantics, instrument hardening, trivia).
>
...
>
> **DONE 2026-07-18 (latest): CMP-AUD-046 CLOSED — shifted exports now show diffs under
> the right field.** Ramp Detail & Intersection Detail exports have header labels shifted
> right of their VALUE positions, so cross-env reported a real Ramp Description change under
> "R/U" and an Intersection INT Type change under "INT Eff-Date" (diff fired correctly by
> position, but pointed at the wrong attribute). Per-position census of the real 7.9 exports
> (RD/ID Excel + converted PDF) pinned each position's correct label: **Ramp Detail (Excel +
> PDF)** now applies a position-authoritative `force_header` (`_RD_ENV_HEADER` + On/Off/Ramp
> Type for the PDF), like Highway Log's corrected labels; **Intersection Detail**'s current
> edition realigned its labels over their values — the Excel side was already fixed by
> `_id_canonical_header` (032/048), and the **PDF** side (no canonicalizer → shifted legacy
> labels) now pins the same one. Values compare BY POSITION so no diff COUNT changes — display
> only. New `check_compare_env_field_labels` proves end-to-end: a Description change shows
> under Description (not R/U), an INT Type change under INT Type (not INT Eff-Date). `1edca5f`.
> Offline gate **132/132** + ruff clean. **NEXT (bucket D — LAST): 022** (duplicate normalized
> categories — the `counts_from_rows` double-count; mind the CONTROL-TYPES signal fold, key on
> (block, pre-fold-code); see the takeover memory's 022 scope). Then buckets B/E/G/H/I.
>
> **DONE 2026-07-18: CMP-AUD-032 CLOSED — BUCKET C COMPLETE (flat cross-env
> schema pinning).** The flat cross-env families locked their header from the
> alphabetically-first readable file, so two identically-malformed or legacy sides
> compared as a clean MATCH (Highway Detail never called its exact recognizer; ID's
> canonicalizer returned an unrecognized layout unchanged). Fix: every flat family now
> pins its EXACT current export schema via a `header_canonicalizer` (the pattern Highway
> Log already used) — a shared `_flat_header_recognizer` factory over the vs-TSN
> `_TSMIS_HEADER[1:]` (RD/HSL/ID, unnamed columns included) / `highway_detail_columns.HEADER`
> (HD), returning None for anything else → an "unrecognized layout" refusal; `_id_canonical_header`
> refuses non-edition layouts (still bridges current↔legacy). The two PDF-conversion loaders
> (HSL-PDF reuses the Excel recognizer; RD-PDF pins its own 13-col print header with the
> On/Off + Ramp Type sentinels) pin the same way via the config path. **Census:** the bound
> 7.9 statewide exports (126 RD / 252 HSL / 252 HD / 217 ID) + both converted PDF headers —
> one consistent shape each, none carrying a leading Route, so pinning drops no real file.
> Two commits (`528e91c` XLSX + `62d1593` PDF). New `check_compare_env_flat_schema` (per-family
> recognizer matrix + end-to-end refuse/accept + PDF coverage); 5 fixtures
> (`check_compare_ramp_detail`, `_highway_sequence`, `_env_intersection`, `_baseline_matrix`,
> `check_matrix`) rebuilt from fake short headers onto real layouts (red→green). Offline gate
> **131/131** + ruff clean. **NEXT (bucket D): 046** (position-shifted header projection) +
> **022** (duplicate normalized categories — the `counts_from_rows` double-count; mind the
> CONTROL-TYPES signal fold, key on (block, pre-fold-code); see the takeover memory's 022 scope).
>
> **DONE 2026-07-17: CMP-AUD-019 CLOSED — the Ramp Summary producer now
...
> comparison could certify a clean match while a whole route present on one side was
> invisible (it errored only when the WHOLE side was empty). **Census** (the decision
> gate — legitimate data-less route vs anomaly): a read-only sweep of all 756 real
> per-route XLSX exports in the bound 7.9 corpora (RD 126 / HSL 252 / HL 252 ssor-prod +
> HD 252 ars-prod; min data-row count 1/5/2/2) found **ZERO header-only files** → a
> data-less export is anomalous. Fix: `_load_xlsx_side` now discloses a `count == 0` file
> LOUDLY into the same `skipped` incompleteness channel as the 030/031 skips (naming the
> route), so `_coerce_loaded_side` marks the comparison `partial` and the route is never
> erased; the whole-empty-side raise is unchanged. Never false-fires (no real export is
> header-only). Red→green (pre-fix `skipped == []`); 3 new tests in
> `check_compare_env_route_universe.py` (disclosure, sole-file error, the finding's exact
> end-to-end fixture → `completion="partial"` with route 002 named). Offline gate
> **128/128** + ruff clean. **NEXT: the MER-059 census** (route 059 p5 em-dash
> group-header-shaped line — scan all 252 HL prints, classify with evidence before any
> rule), then buckets B/D/E/G/H/I.
...
>
> **DONE 2026-07-17: CMP-AUD-029 CLOSED — the generic cross-env XLSX discovery
> now filters Office owner-lock stubs.** `_find_input_dir` (the single glob
> chokepoint `_load_xlsx_side` + the Ramp/Intersection Summary + PDF-side loaders
> all pass through) dropped `~$`-prefixed names, matching the owner-lock filter the
> shared consolidator / Intersection Summary / `baseline_matrix` / `day_matrix` /
> `tsn_library` / `validation` already applied. A `~$` stub could open-fail into a
> skip and mark identical exports incomplete (then CMP-AUD-017 could cache the
> matrix cell green). Red→green in `check_compare_env_route_universe`
> (`test_029_owner_lock_ignored`: pre-fix a `~$hs_route_001.xlsx` stub → "could not

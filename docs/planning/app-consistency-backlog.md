...
| 10 | **Overall export consistency** — all exports done basically the same regardless of surface (except the Everything matrix's own store, which is intentional). Matrices should be a *front-end* over the same comparisons done manually. | ARCH | High — cross-cutting | Design decision (Claude); the unifying principle for 9/11 |

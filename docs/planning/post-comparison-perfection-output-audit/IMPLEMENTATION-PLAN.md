...
| 3 | **RB-3** | HF-04 | Ramp Detail — restores 9 comparison placements that produce nothing today | None | `hotfix/rb-3-ramp-detail-layout` | **DENIED — RETURN TO IMPLEMENTATION** (`RB3-R1-EG-002`) |
...
| **RB-3** | HF-04 | 001, 012 | 2 | **DENIED — RETURN TO IMPLEMENTATION** (`RB3-R1-EG-002`) |
...
`RB-3/BUNDLE.md` now exists because RB-3 is `READY`.
...
1 denied `RB3-R1-EG-001`; the implementation then committed a complete
exact-head manifest and verifier. Review 1 re-review closed that gap after both
present-byte verification paths passed, but denied one narrower evidence gap as
`RB3-R1-EG-002`: explicitly requested corpus/archive verification skips absent
declared roots, results, TSN raw input, and archives without failing. Resume
Prompt 04 on the existing RB-3 branch to make the verifier fail closed and add
negative checks. Retain the bound acceptance set; do not begin Review 2.

...
| 4 | **RB-4** | HF-05 + HF-10 | Evidence, end to end — **AMENDED by the owner 2026-08-05**: print crops on the four `_pdf` report families only (12 cells), refused at the engine boundary everywhere else | None | `hotfix/rb-4-evidence` | **DENIED — RETURN TO IMPLEMENTATION** (`RB4-R1-001`, reviewed through `3cda54f`) |
...
| **RB-4** | HF-05, HF-10 | 004, 005, 006, 007 | 4 | **DENIED — RETURN TO IMPLEMENTATION** (`RB4-R1-001`) |
...
RB-4 was implemented on `hotfix/rb-4-evidence`, with acceptance runtime
`f4b55f2`, and reviewed through `3cda54f`. Codex Review 1 denied one concrete
HF-10 visual acceptance failure as `RB4-R1-001`: one retained blank-cell crop
is not human-verifiable, so the required 100% visual pass rate is not met.
Resume Prompt 04 on the existing branch; do not begin Review 2 or merge.

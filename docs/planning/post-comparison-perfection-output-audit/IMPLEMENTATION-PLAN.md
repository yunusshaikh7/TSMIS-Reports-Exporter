...
| 4 | **RB-4** | HF-05 + HF-10 | Evidence, end to end — **AMENDED by the owner 2026-08-05 and 2026-08-09**: print crops on the four `_pdf` report families only; anchorless blanks from the print's own cell geometry are disclosed limitations | None | `hotfix/rb-4-evidence` | **REVIEW 2 DENIED — RETURN TO IMPLEMENTATION** (`RB4-R2-001`, reviewed at `f714f1b`) |
...
| **RB-4** | HF-05, HF-10 | 004, 005, 006, 007 | 4 | **REVIEW 2 DENIED — RETURN TO IMPLEMENTATION** (`RB4-R2-001`) |
...
| Review 2 | **Codex** — a second, separate chat that must challenge review 1 and re-derive from source, never copy it |
| Status | **Inherits RB-4: REVIEW 2 DENIED — RETURN TO IMPLEMENTATION** |
...
RB-4 remains implemented on `hotfix/rb-4-evidence` at runtime `f4b55f2`.
Codex Review 1 re-reviewed owner-ruling head `14475ff`, closed `RB4-R1-001` by
the amended HF-10 definition rather than by code, and approved every criterion.
Codex Review 2 then denied `RB4-R2-001`: after valid comparison provenance is
accepted, missing required TSMIS/TSN print sets raise through keep-last-good and
leave the prior canonical evidence workbook, image directory, and manifest in
place, contrary to HF-05 criterion 1's zero-artifact print-binding rule. RB-4 is
**REVIEW 2 DENIED — RETURN TO IMPLEMENTATION**. Resume Prompt 04 on the existing
branch; do not merge.
